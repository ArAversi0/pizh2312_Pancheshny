...
void TextMetadata::display() const {
    ...
    std::cout << "[*] " << identifier;
    std::string desc = getBlockDescription(identifier);
    if (!desc.empty()) {
        std::cout << " (" << desc << ")";
    }
...
void UserComment::display() const {
    ...
    if (textStart == content.end() || textStart + 1 >= content.end()) {
        std::cout << "[?] Invalid comment format";
        return;
    }
...
void parseMetadataBlocks(std::ifstream& file, uint32_t totalSize) {
    ...
    while (processed + 10 <= totalSize) {
        ...
        if (file.gcount() != 4 || blockId == "\0\0\0\0") {
            break;
        }

        std::array<uint8_t, 4> sizeData{};
        file.read(reinterpret_cast<char*>(sizeData.data()), 4);
        uint32_t blockSize = calculateFrameSize(sizeData);
        ...
        if (file.gcount() != static_cast<int>(blockSize)) {
            break;
        }
