...
{
    // Конструкторы класса ArgParser позволяют создавать объект парсера аргументов
    // с указанием имени программы или без параметров.
    // help_requested_ инициализируется в false, указывая, что помощь по умолчанию не запрашивалась.
    ...
    // Добавляет строковый аргумент с длинным именем.
    // Инициализирует значение строки и сохраняет его в список аргументов.
    ...
    // Добавляет флаг (булевый аргумент), который может быть установлен или нет.
    // Используется для параметров типа --verbose или -h.
    ...
    // Метод StoreValue связывает строковую переменную с последним добавленным аргументом.
    // Это позволяет сохранять значение, переданное в аргументе командной строки, напрямую в переменную.
    ...
    // Метод MultiValue указывает, что аргумент может принимать несколько значений.
    // min_values определяет минимальное количество значений, необходимых для корректной обработки.
    ...
    // Устанавливает аргумент как позиционный.
    // Позиционные аргументы задаются без ключей (например, ./prog file.txt).
    ...
    // Метод Default задает значение по умолчанию для строкового аргумента.
    // Если значение не задано пользователем, то используется это значение.
    ...
    {
        // Ищем аргумент по имени и возвращаем его строковое значение,
        // если оно задано. В противном случае — значение по умолчанию.
    ...
    {
        // Ищем аргумент по имени и возвращаем первое значение из вектора целых чисел,
        // если оно задано. В противном случае — значение по умолчанию.
        for (const auto &arg : arguments_)
        {
    ...
    {
        // Проверяем, был ли установлен флаг по имени аргумента.
        // Если да — возвращаем его значение. Иначе — значение по умолчанию или false.
        for (const auto &arg : arguments_)
        {
    ...
    {
        // Добавляем аргумент для отображения справки (-h или --help),
        // который устанавливает флаг help_requested_, если был задан.
    ...
    {
        // Возвращает флаг, был ли запрошен вывод справки.
        return help_requested_;
    }

    std::string ArgParser::HelpDescription() const
    {
        // Генерирует строку с описанием всех доступных аргументов
        // и выводит их в удобочитаемом виде для пользователя.
    ...
    {
        // Обрабатывает аргументы вида --ключ=значение.
        // Поддерживает как флаги (--flag), так и аргументы со значением (--key=value).
    ...
    {
        // Обрабатывает короткие аргументы, например -f или -oValue.
    ...
    {
        // Обрабатывает позиционные аргументы (без префиксов), например `./prog 42`.
    ...
    {
        // Выполняет финальную проверку после парсинга аргументов.
        // Проверка обязательных аргументов, минимального числа значений,
        // установка значений по умолчанию и проверка флага помощи.
        for (const auto &argument : arguments_)
        {
            if (argument.is_positional && argument.min_values > 0 && argument.value_int && argument.value_int->size() < argument.min_values)
            {
