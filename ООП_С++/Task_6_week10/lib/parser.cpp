...
namespace omfl
{
    ...
    {
        if (IsInt())
            return get<int32_t>(value_);
        if (IsFloat())
            return static_cast<int32_t>(get<float>(value_));
    ...
    {
        if (IsFloat())
            return get<float>(value_);
        if (IsInt())
            return static_cast<float>(get<int32_t>(value_));
    ...
    {
        if (!IsArray())
            return OMFLSection::empty_value_;
                    ...
                    if (!array_content.empty())
                    {
                        ...
                        while (getline(array_stream, element, ','))
                        {
                            ...
                            else if (element.find('.') != string::npos)
                            {
                                ...
                                if (result.ec != errc() || *result.ptr != '\0')
                                    return false;
